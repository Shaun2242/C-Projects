...
// Constructor of UJBlurFilter chaining the base class constructor
UJBlurFilter::UJBlurFilter() : UJFilter() {
    int tempArray[3][3] = {{1, 2, 1},
                           {2, 4, 2},
                           {1, 2, 1}};
...
void UJBlurFilter::applyFilter(CCImage &object, int intNumPasses) {
            ...
            for(int c = 1; c < intCols - 1; c++) {
                ...
                for(int rk = -1; rk <= 1; rk++) {
                    for(int ck = -1; ck <= 1; ck++) {
                        sumR = sumR + ImageCopy[r + rk][c + ck].intRed * _Kernel[rk+1][ck+1];
                        sumG = sumG + ImageCopy[r + rk][c + ck].intGreen * _Kernel[rk+1][ck+1];
                        sumB = sumB + ImageCopy[r + rk][c + ck].intBlue * _Kernel[rk+1][ck+1];
                    }
                }
                AveR = sumR / 9;
                AveG = sumG / 9;
